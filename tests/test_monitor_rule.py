...
class TestMonitorRule:
    ...
    def test_create_monitor_rule(self, auth_client, test_advertiser):
        resp = auth_client.post("/api/ads/monitor-rules/", {
    ...
    def test_list_monitor_rules_by_advertiser(
        self, auth_client, test_advertiser, test_rule
    ):
        ...
        resp = auth_client.get(
            f"/api/ads/monitor-rules/?advertiser_id={test_advertiser.id}"
        )
        assert resp.status_code == 200
    ...
    def test_list_monitor_rules_advertiser_not_found(self, auth_client):
        resp = auth_client.get(
            f"/api/ads/monitor-rules/?advertiser_id={uuid.uuid4()}"
        )
